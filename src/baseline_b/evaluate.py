"""
Evaluate Module for Baseline B

Kill gates and comprehensive metrics per spec §10.
...
from .backtest import BacktestResult, Backtester, load_model
...
class Evaluator:
    """
    Evaluates Baseline B strategy against kill gates.

    Kill Gates (per spec §10):
    1. net_CAGR > 0
    2. MaxDD <= 15%
    3. trade_days >= 30
    """
    ...
    def _compute_gross_sharpe(self, backtest: BacktestResult) -> float:
        ...
        gross_returns = np.array([d.gross_return for d in backtest.daily_results])
    ...
    def evaluate(self, backtest: BacktestResult) -> EvaluationResult:
        ...
        # Gate 3: trade_days >= 30
        gate3_passed = backtest.trade_days >= self.MIN_TRADE_DAYS
        kill_gates.append(KillGateResult(
            name="trade_days",
            threshold=">= 30",
            actual=float(backtest.trade_days),
            passed=gate3_passed,
            message=f"Trade days = {backtest.trade_days} {'✅ PASS' if gate3_passed else '❌ FAIL'}",
        ))
...
def load_and_evaluate(
    ...
    # Run backtest
    backtest_kwargs = backtest_config or {}
...
def run_full_evaluation(
    ...
    # Run backtests
    backtest_kwargs = backtest_config or {}
    backtester = Backtester(model=model, scaler=scaler, verbose=verbose, **backtest_kwargs)
    backtests = {}
    for split in splits:
    ...
    if verbose:
        ...
        for split, result in results.items():
            ...
            print(f"{split}: {status} | CAGR={result.cagr*100:.2f}% | Sharpe={result.net_sharpe:.3f} | MaxDD={result.backtest.max_drawdown*100:.2f}%")
