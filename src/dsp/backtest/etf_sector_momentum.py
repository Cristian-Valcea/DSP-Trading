"""
Backtest for Equity Sector ETF Long/Short Momentum.

Strategy
--------
Monthly sector rotation with L/S capability:
1. Universe: 11 SPDR Sector ETFs (XLB, XLC, XLE, XLF, XLI, XLK, XLP, XLRE, XLU, XLV, XLY)
2. Monthly rebalance (first trading day of month)
3. Rank by 12-1 momentum
4. Long top N sectors, Short bottom N sectors (or long-only if shorting disabled)
5. Equal-weight within long/short buckets
6. Vol-target the portfolio to 8% annual volatility

Kill-Test Purpose
-----------------
Test if sector momentum (a well-documented factor) produces tradable edge
with ETFs. No survivorship bias since sector ETFs don't delist.
"""
...
LONG_COUNT = 3         # Long top N sectors
SHORT_COUNT = 3        # Short bottom N sectors (0 = long-only)
...
class ETFSectorMomentumBacktester:
    """
    Sector ETF Long/Short Momentum backtester.

    No survivorship bias - sector ETFs are stable, liquid, don't delist.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._client_id = client_id or (config.ibkr.client_id + 5002)
    ...
    async def run(
        ...
        *,
        ...
        """
        Run the sector momentum backtest.

        Args:
            short_count: Number of sectors to short (0 = long-only mode)
        """
        all_symbols = SECTOR_ETFS + ["SPY"]  # SPY for correlation calc
        ...
        try:
            ...
            positions: Dict[str, int] = {s: 0 for s in SECTOR_ETFS}
            ...
            def _equity(dt: date, col: str) -> float:
                """Calculate equity including short positions."""
                total = cash
                for sym, qty in positions.items():
                    if qty == 0:
                        continue
                    px = _price(sym, dt, col)
                    # Long: add value, Short: subtract value (but qty is already negative)
                    total += qty * px
                return float(total)
            ...
            def _asset_vol(sym: str, signal_dt: date) -> float:
            ...
            def _compute_weights(signal_dt: date) -> Dict[str, float]:
                """
                Sector momentum: long top N, short bottom N.
                """
                ...
                for sym in SECTOR_ETFS:
                    ...
                    if m is not None:
                        mom_scores[sym] = m
                        vols[sym] = _asset_vol(sym, signal_dt)

                if len(mom_scores) < long_count + short_count:
                ...
                longs = [sym for sym, _ in ranked[:long_count]]
                shorts = [sym for sym, _ in ranked[-short_count:]] if short_count > 0 else []

                # Equal weight within buckets
                long_weight = 1.0 / len(longs) if longs else 0.0
                short_weight = -1.0 / len(shorts) if shorts else 0.0

                weights = {}
                for sym in longs:
                    weights[sym] = long_weight
                for sym in shorts:
                    weights[sym] = short_weight

                # Vol-target scaling
                # Estimate portfolio vol (simple: sum of |weight| * vol)
                port_vol = sum(abs(weights.get(s, 0)) * vols.get(s, 0.15) for s in SECTOR_ETFS)

                if port_vol > 0:
                    scale = vol_target / port_vol
                    scale = min(scale, 2.0)  # Don't lever more than 2x
            ...
            for i, dt in enumerate(dates):
                ...
                if _is_first_of_month(dt):
                    ...
                    monthly_holdings.append({
                        ...
                        **{f"w_{s}": weights.get(s, 0.0) for s in SECTOR_ETFS}
                    })

                    # Convert to target shares
                    targets: Dict[str, int] = {s: 0 for s in SECTOR_ETFS}
                    for sym, w in weights.items():
                        ...
                        if px <= 0:
                            continue
                        # Positive weight -> long, Negative weight -> short (negative shares)
                    ...
                    for sym in SECTOR_ETFS:
                        ...
                        if delta > 0:
                            # Either buying to go long or buying to cover short
                            side = "BUY"
                        else:
                            # Either selling long or selling short
                            side = "SELL"
...
def _print_kill_table(equity: pd.Series) -> Dict[str, float]:
    """Print kill-test windows and return 2022-2024 metrics."""
    windows = [
...
def _print_kill_verdict(metrics: Dict[str, float], mode: str) -> str:
    ...
    # Drawdown check (more lenient for L/S)
    dd_threshold = -0.30 if mode == "L/S" else -0.25
    if max_dd < dd_threshold:
        verdicts.append(f"❌ FAIL: Max DD {max_dd*100:.1f}% exceeds {dd_threshold*100:.0f}% threshold")
    elif max_dd < -0.15:
    ...
    if vol < 0.04:
        ...
        verdicts.append(f"⚠️  WARN: Vol {vol*100:.1f}% exceeds target range")
    else:
        verdicts.append(f"✅ PASS: Vol {vol*100:.1f}% in target range")

    # Overall verdict
    if sharpe < 0 or max_dd < dd_threshold:
...
async def _async_main() -> int:
    parser = argparse.ArgumentParser(description="Backtest Sector ETF Long/Short Momentum")
    ...
    parser.add_argument("--long-count", type=int, default=LONG_COUNT, help=f"Sectors to go long (default: {LONG_COUNT})")
    parser.add_argument("--short-count", type=int, default=SHORT_COUNT, help=f"Sectors to short (default: {SHORT_COUNT}, 0=long-only)")
    ...
    mode = "L/S" if args.short_count > 0 else "Long-only"

    config = load_config(args.config, strict=False)
    bt = ETFSectorMomentumBacktester(config)
    result = await bt.run(
    ...
    print("\n" + "="*60)
    print(f"SECTOR ETF MOMENTUM BACKTEST RESULTS ({mode})")
    print("="*60)
    print(f"\nStrategy: Long {args.long_count}, Short {args.short_count}, vol-target {args.vol_target*100:.0f}%")
    ...
    print(f"\n--- Kill Test Verdict (2022-2024, {mode}) ---")
    print(_print_kill_verdict(kill_metrics, mode))
